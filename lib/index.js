...
var flatten = function(options) {
    ...
    files.forEach(function(filename, index) {
        ...
        if (fs.lstatSync(licenseFile).isFile()) {
            ...
            if (index === 0) {
                ...
                // https://raw.githubusercontent.com/caolan/async/v3.2.3/LICENSE
                // https://github.com/caolan/async/blob/v3.2.3/LICENSE

                // https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/LICENSE
                // https://raw.githubusercontent.com/DefinitelyTyped/DefinitelyTyped/v2.4.2/LICENSE

                // https://raw.githubusercontent.com/emotion-js/emotion/v10.0.6/LICENSE
                if (include("licenseFileRepo")) {
                    const paths = moduleInfo.licenseFile.split(path.sep);
                    const filename = paths[paths.length - 1];
                    moduleInfo.licenseFileRepo = `${moduleInfo.repository}/blob/v${moduleInfo.version}/${filename}`;
                }
            }
        }
    });
    ...
    if (moduleInfo.repository) {
        const paths = moduleInfo.licenseFile.split(path.sep);
        const filename = paths[paths.length - 1];
        moduleInfo.licenseFileRepo = extractLicense(moduleInfo.repository, `v${moduleInfo.version}`, filename);
        if (!moduleInfo.licenseFileRepo) {
            moduleInfo.licenseFileRepo = extractLicense(moduleInfo.repository, "master", filename);
        }
        if (!moduleInfo.licenseFileRepo) {
            moduleInfo.licenseFileRepo = extractLicense(moduleInfo.repository, moduleInfo.version, filename);
        }
        if (!moduleInfo.licenseFileRepo) {
            moduleInfo.licenseFileRepo = extractLicense(moduleInfo.repository, "main", filename);
        }

        console.log("moduleInfo.licenseFileRepo: ", moduleInfo.licenseFileRepo);
    }
...
function extractLicense(repository, version, filename) {
    const remotePath = `${repository}/blob/${version}/${filename}`;
    const s = remotePath.split("github.com");
    let remoteRawPath = `https://raw.githubusercontent.com${s[1]}`;
    remoteRawPath = remoteRawPath.replace("/blob/", "/");
    const buff = exec(`curl "${remoteRawPath}"`);
    if (buff.toString() === "404: Not Found") {
